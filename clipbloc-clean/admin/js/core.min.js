...
if(typeof language==="undefined"){
   ...
   //var language=lan;
    ...
    var languages = ["es","en"];
    var languagesRots = {"es":{"es":"Castellano","en":"Inglés"},"en":{"es":"Spanish","en":"English"}};
    var showLanDiv=false;
    var lastLanSelected = null;
    var lastLanSelectedClass = null;
    var languageDiv=document.createElement("div");
    languageDiv.id="languageDiv";
    var languageDivInner=document.createElement("div");
    languageDivInner.id="languageDivInner";
    languageDiv.onclick=function(){
        languageDiv.style.left="-350px";
        showLanDiv=false;
    }
    var oriDispLan=null;
    for( var j in languagesRots){
        for( var k in languagesRots[j]){
            var newLanPB=document.createElement("div");
            newLanPB.className="newLanPB "+j;
            newLanPB.setAttribute("selLan",k);
            newLanPB.innerHTML=languagesRots[j][k];
            newLanPB.onclick=function(event){
                if(lastLanSelected){
                    lastLanSelected.className=lastLanSelectedClass;
                    lastLanSelectedClass=event.target.className;
                    lastLanSelected=event.target;
                }
                language=event.target.getAttribute("selLan");
                var allLanSel=document.getElementsByClassName(language);
                for(var j in allLanSel){
                    if(typeof allLanSel[j] === "object" && allLanSel[j].getAttribute("selLan")===language){
                        if(lastLanSelected){
                            allLanSel[j].className=lastLanSelectedClass + " newLanPBSelected";
                        }else{
                            allLanSel[j].className+=" newLanPBSelected";
                        }
                    }
                }
                changeLanguage();
            }
            oriDispLan=newLanPB.style.display;
            languageDivInner.appendChild(newLanPB);
        }
    }
    languageDiv.appendChild(languageDivInner);
    
    var showLanguageDiv=function(event){
        if(showLanDiv){
            languageDiv.style.left="-350px";
        }else{
            languageDiv.style.left="0"
        }
        showLanDiv=!showLanDiv;
    }
    var lastFlag=null;
    var changeLanguage=function(_language,_flag){
        if(_flag){
            if(lastFlag){
                lastFlag.style.opacity="1";
            }
            lastFlag=_flag;
            lastFlag.style.opacity=".5";
        }
        if(_language){
            language=_language;
        }
        for( var j in languages){
            if(languages[j]!==language){
                var rotsLan=document.getElementsByClassName(languages[j]);
                for( k=0;k<=rotsLan.length-1;k++){
                    rotsLan[k].style.display="none";
                }
            }else{
                rotsLan=document.getElementsByClassName(languages[j]);
                for( k=0;k<=rotsLan.length-1;k++){
                    rotsLan[k].style.display=oriDispLan;
                }
            }
        }
    }
if(typeof lanPB!=="undefined"){
lanPB.innerHTML="<span class='es'>Idioma</span><span class='en'>Language</span>";
}
if(typeof privatePB!=="undefined"){
privatePB.innerHTML="<span class='es'>Admin</span><span class='en'>Admin</span>";
}
selLan=function(){
showLanguageDiv();
}
var lanPB=null;
createLanDiv =function(){
    document.body.appendChild(languageDiv);
    lanPB=document.getElementById("lanPB");
    lanPB.onclick=function(){
        document.body.appendChild(languageDiv);
        showLanguageDiv();
    }
    var allLanSel=document.getElementsByClassName(language);
...
controlSecurity=function(){
    ...
    close.className="close";
    close.innerHTML="<img src='images/cancel-circle.png' />";
    close.onclick=function(){ closeDiv(filesDiv);document.body.style.overflow=""; };
     var usersList = "";
...
function users() {
    ...
    self.anadir = function() {
        ...
        message.className="col-xs-3 basicPB";
        savePB=document.getElementById("basicSavePB");
        savePB.className=savePB.className+" basicSavePB_active";
    };
...
function showMessageUsers(_message) {
    message.innerHTML = "Saved";
        message.className=message.className + " message_active";
        savePB.className="col-xs-3 basicPB";
}
...
var save_css=function(data){
    ...
    message.innerHTML="Saved";
    savePB=document.getElementById("basicSavePB");
    savePB.className="col-xs-6 basicPB";
    message.className=message.className + " message_active";
}
...
var HTMLBG = "";
var lastHTML = null;
var divHTMLContainer = null;
var showMenuHTML = false;
getHTML = function(rot, desc, image, url, page, fbToClip) {
    ...
    htmlMenu.className = "htmlMenu";
    htmlMenu.innerHTML = "Menu HTML";
    htmlMenu.onclick = function() {
        showHTMLMenu();
    };
    htmlDiv.appendChild(htmlMenu);
    
    
    
    
    
    divHTMLContainer = document.createElement("div");
    divHTMLContainer.className = "divHTMLContainer"
    htmlDiv.appendChild(divHTMLContainer);
    divHTMLContainer.onclick=function(event){
        showHTMLMenu();
    }
    
    
    var lanHTMLPB = document.createElement("div");
    lanHTMLPB.className = "showHTMLPB";
    lanHTMLPB.innerHTML = "Language";
    lanHTMLPB.onclick = function() {
        showLanguageDiv();
    };
    divHTMLContainer.appendChild(lanHTMLPB);
    ...
    divHTMLContainer.appendChild(showHTMLPB);
    ...
    divHTMLContainer.appendChild(copyHTMLPB);
    ...
    //$("#htmlContent").load("../pages/" + page + ".html?" + refresh, function() {
        ...
        if(document.getElementById("htmlContent").innerHTML.substr(0,4)==="CLIP"){
            document.getElementById("htmlContent").innerHTML="<div id='htmlContent' style='height:100%;position:relative'>\n<div class='basicOuter'><div class='basicInner'>CLIP</div></div>\n</div>";     
        }
        setTimeout(function() {
            changeLanguage();
...
var showHTMLMenu=function(event){
        if(showMenuHTML){
            divHTMLContainer.style.right=-divHTMLContainer.offsetWidth+"px";
        }else{
            divHTMLContainer.style.right="0";
            if(htmlCodesDiv){
                closeCodesDiv(htmlCodesDiv);
            }
        }
        showMenuHTML=!showMenuHTML;
    }
...
showHTML = function(rot, desc, image, url, page, fbToClip) {
    ...
    if (contentHTML.indexOf("htmlContent")<=-1 || contentHTML.indexOf("clipblocImport::")) {
        HTMLInput.value = "<div id='htmlContent' style='height:100%;position:relative'>\n<div class='basicOuter'><div class='basicInner'>CLIP</div></div>\n</div>";
    } else {
    ...
    saveHTML.onclick = function() {
        alert("here");
        saveHTMLPage(HTMLInput.value, rot, desc, image, url, page, htmlDiv, fbToClip);
    };
    saveHTML.onclick = function() {
        if(user==="test"){
            showTestModeDiv();
...
saveHTMLPage = function(input, rot, desc, image, url, page, div, fbToClip) {
    ...
    if(document.getElementById("htmlContent")){
        document.getElementById("htmlContent").innerHTML = input;
    }
    var CUERPO = input;
...
var htmlCodesDiv =null;
copyHTML = function() {
    copyHTMLMode = true;
    if(!document.getElementById("htmlCodesDiv")){
        
    
    htmlCodesDiv = document.createElement("div");
...
function sections(){
    ...
    self.ini = function() {
        var refresh = new Date().getTime();
        $.getJSON("../controller/environment/json/groups.json?"+ refresh, function(data) {
        }).complete(function(dataTabs) {
            horMenu=document.getElementById("menuContainer");
            menuAdmin=document.getElementById("menu");
            ...
            if(!initalized){
                if (document.getElementById("userSession")) {
                        ...
                        //menu access
                        menuPB = document.createElement("div");
                        menuPB.id = "menuPB";
                        menuPB.innerHTML = "menu";
                        menuPB.onclick = function(){
                            showMenu();
                        }
                        document.body.appendChild(menuPB);
                        
                        ini.click();
                        horMenu.onclick = function(){
                            showMenu();
                        }
...
function menu_level_2(){
    ...
    self.ini = function(tabs) {
        ...
        $.getJSON(pathJSON+refresh, function(data) {
            ...
            setTimeout(function() {
                ...
                endTrans = function() {
                    if (pageFocus.offsetLeft===0 && lastPage) {
                        all.removeChild(lastPage);
                        savePB=document.getElementById("basicSavePB");
                        message=document.getElementById("message");
                        endTrans = null;
                        lastPage = pageFocus;
                    }else if(pageFocus.offsetLeft===0 && !iniLast){
                        endTrans = null;
                        iniLast = true;
                        lastPage = pageFocus;
                    }                
   ...
   self.reset=function(tab){
    ...
    self.anadir = function() {
        ...
        message=document.getElementById("message");
        message.innerHTML="Waiting...";
        message.className="col-xs-6 basicPB";
        scrollToBottom(document.body,document.body.scrollHeight,1000,htmlDate);
        savePB=document.getElementById("basicSavePB");
        savePB.className=savePB.className+" basicSavePB_active";
        ...
        var urlIni=document.URL.split("/admin")[0];
            ...
            var fbSharer=urlIni+"/pages/"+htmlDate+"/facebook.html?refresh="+refresh;
        
            var fbToClip=urlIni+"/index.php?view="+htmlDate;
        
        //getHTML(tabs.rot,tabs.desc,tabs.image,fbSharer,tabs.date,fbToClip);
        
        saveHTMLPage("<div id='htmlContent' style='height:100%;position:relative'>\n<div class='basicOuter'><div class='basicInner'>CLIP</div></div>\n</div>", "New","No define",urlIni+"/pages/"+htmlDate+"/fbImg.jpg?refresh="+refresh,fbSharer,htmlDate,fbToClip);
    ...
    self.createHTML = function(tabs) {
        ...
        //message.style.display="none";
        //message.style.background="initial";
    ...
    self.guardarGroup = function() {
     ...
     self.extend = function(tabs) {
        ...
        if (selTab.style.height === "550px") {
            selTab.style.height = "0";
        } else {
            if (document.getElementById(lastExtend)) {
                document.getElementById(lastExtend).style.height = "0";
            }
            selTab.style.height = "550px";
...
var menuPBX=null;
showMenu=function(){
    if(horMenu.style.right !== "-15px"){
        horMenu.style.right="-15px";
        menuPBX=menuPB.style.right;
        menuPB.style.right=-horMenu.offsetWidth+"px";
    }else{
      horMenu.style.right=-horMenu.offsetWidth+"px";
      menuPB.style.right=menuPBX;
    }
...
scrollToTop=function (scrollDuration) {
    ...
    scrollToBottom = function (element, to, duration, target) {
    var start = element.scrollTop,
        change = to - start,
        increment = 20;

    var animateScroll = function(elapsedTime,target) {        
        elapsedTime += increment;
        var position = easeInOut(elapsedTime, start, change, duration);                        
        element.scrollTop = position; 
        if (elapsedTime < duration) {
            setTimeout(function() {
                animateScroll(elapsedTime,target);
            }, increment);
        }else{
            if(target){
               document.getElementById("rot_"+target).parentNode.style.background="orangered";
            }
            savePB.click();
...
focusTextReg = function(){
    message.className="col-xs-3 basicPB";
    message.innerHTML="Waiting...";
    savePB=document.getElementById("basicSavePB");
    savePB.className=savePB.className+" basicSavePB_active";
}
changeRot = function(_target){
    _target.addEventListener("input", function(){
        document.getElementById("rot_"+_target.id.split("_")[1]).innerHTML=_target.value;
        var start = _target.selectionStart,
        end = _target.selectionEnd;
        // restore from variables...
        _target.value=_target.value.replace("\"","'");
        _target.setSelectionRange(start, end);
    });
}
var testModeDiv = null;
showTestModeDiv = function(){
    testModeDiv=document.createElement("div");
    testModeDiv.id="testModeDiv";
    testModeDiv.innerHTML='<div class="basicOuter"><div class="basicInner"><div id="statusTest"><span class="en">'+rots["testMode"][language]+'</span></div><div id="closeTestModeDivPB"><span class="es" style="display: none;">Continuar</span><span class="en">Continue</span></div></div></div>';
    testModeDiv.onclick=function(){
        closeTestModeDiv();
    }
    document.body.appendChild(testModeDiv);
    setTimeout(function() {
        testModeDiv.style.left="0";
        endTrans = function() {
                endTrans = null;
        };
    }, 10);
...
closeTestModeDiv=function(){
    if(testModeDiv){
        testModeDiv.style.left="-100%";
        endTrans = function() {
            if(testModeDiv.offsetLeft==="-100%"){
                endTrans = null;
                document.body.removeChild(testModeDiv);
                testModeDiv=null;
            }
